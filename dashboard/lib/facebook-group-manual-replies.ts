...
export const FACEBOOK_GROUP_STATE_DIR = path.join(process.cwd(), 'state', 'facebook-group-finder', 'jobs');
const FACEBOOK_GROUP_ROOT_DIR = path.join(process.cwd(), 'state', 'facebook-group-finder');
const PENDING_MANUAL_REPLIES_FILE = path.join(FACEBOOK_GROUP_ROOT_DIR, '_pending-manual-replies.json');
...
function ensureStateDir() {
  fs.mkdirSync(FACEBOOK_GROUP_ROOT_DIR, { recursive: true });
}
...
export function setPendingFacebookGroupManualReplyForState(
...
export function updatePendingFacebookGroupManualReplyPromptMessageId(entry: PendingManualReply | null | undefined, promptMessageId: number | null | undefined) {
...
export function findPendingFacebookGroupManualReply(
...
export function clearPendingFacebookGroupManualReply(entry: PendingManualReply | null | undefined) {
  if (!entry) return;
  const store = pruneExpiredPending(readPendingStore());
...
export function findLatestAwaitingFacebookGroupManualReplyState(chatId: string | number, threadId: string | number) {
  const normalizedChatId = String(chatId || '').trim();
  const normalizedThreadId = Number(threadId || 0);
  if (!normalizedChatId || !normalizedThreadId || !fs.existsSync(FACEBOOK_GROUP_STATE_DIR)) return null;

  const candidates: FacebookGroupState[] = [];
  for (const fileName of fs.readdirSync(FACEBOOK_GROUP_STATE_DIR)) {
    if (!fileName.endsWith('.json') || fileName.startsWith('_')) continue;

    try {
      const state = JSON.parse(fs.readFileSync(path.join(FACEBOOK_GROUP_STATE_DIR, fileName), 'utf8')) as FacebookGroupState;
      if (
...
export function normalizeFacebookGroupManualReplyText(rawText: string, state?: FacebookGroupState | null) {
  let replyText = String(rawText || '').trim();
  replyText = replyText.replace(/^\/fbreply(?:@\w+)?\s*/i, '').trim();
